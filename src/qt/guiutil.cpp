...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinClassic address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BitcoinClassicAddressEntryValidator(parent));
    widget->setCheckValidator(new BitcoinClassicAddressCheckValidator(parent));
}

bool parseBitcoinClassicURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinClassicUnits::parse(BitcoinClassicUnits::BCC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBitcoinClassicURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBitcoinClassicURI(uriInstance, out);
}

QString formatBitcoinClassicURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinClassicUnits::format(BitcoinClassicUnits::BCC, info.amount, false, BitcoinClassicUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinClassic.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinClassic (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BitcoinClassic (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinClassic*.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitcoinClassic\n";
        else
            optionFile << strprintf("Name=BitcoinClassic (%s)\n", chain);
